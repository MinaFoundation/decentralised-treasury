...
const DEFAULT_MINA_NODE_URL = "http://127.0.0.1:8080/graphql";
const DEFAULT_ARCHIVE_NODE_URL = "http://host.docker.internal:8282/graphql";
const DEFAULT_BROWSER_MINA_NODE_URL = "/mina/graphql";
const DEFAULT_NETWORK_ID = "LOCALNET";
...
function createProgram() {
  return new Command()
    ...
    .option(
      ...
      "Archive GraphQL URL used by indexer services",
...
function buildWebEnvFile(options, roles, browserProverEnv = {}) {
  ...
  const lines = [
    ...
    formatEnvLine("NEXT_PUBLIC_TREASURY_API_URL", "/api"),
    formatEnvLine("NEXT_PUBLIC_INDEXER_API_URL", "/indexer"),
    formatEnvLine("NEXT_PUBLIC_PROCESSOR_API_URL", "/processor"),
...
async function main() {
  ...
  options.nextPublicMinaNodeUrl =
    options.nextPublicMinaNodeUrl ?? options.minaNodeUrl;
