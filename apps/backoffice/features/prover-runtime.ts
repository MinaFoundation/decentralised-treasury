...
function parseJsonValue(value: string | undefined, name: string): unknown {
  if (!value?.trim())
    throw new Error(`${name} is required for proposal toggling.`);
...
function minaToNanomina(value: string): number {
  if (!/^\d+(?:\.\d{0,9})?$/.test(value.trim())) {
    throw new Error(
      "The fee must be a positive MINA value with at most nine decimal places.",
    );
  }
...
function createSignatureSet(
  loaded: Awaited<ReturnType<typeof modules>>,
...
export async function buildAndProveBreakGlassTransaction(input: {
  ...
  if (senderFetch.error)
    throw new Error(
      `Fee payer account was not found: ${String(senderFetch.error)}`,
    );
  ...
  if (
    !onChainCommitment ||
    onChainCommitment.toString() !== operation.multisigCommitment
  ) {
    throw new Error(
      "The on-chain multisig commitment changed. Create a new operation.",
    );
  }
  ...
  if (operation.kind === "toggleProposal") {
    ...
    const configuredController =
      await toggleOwner.pauseControllerPublicKey.fetch();
    if (!configuredController?.equals(controllerKey).toBoolean()) {
      throw new Error(
        "The treasury owner references another pause controller.",
      );
    }
    ...
    if (proposalFetch.error) {
      throw new Error(
        `Proposal account was not found: ${String(proposalFetch.error)}`,
      );
    }
    ...
    if (
      ...
      expectedStatusBefore !== operation.proposalStatusBefore
    ) {
    ...
    async () => {
      if (operation.kind === "pauseTreasury") {
        ...
        if (!operation.nextMultisigCommitment) {
          throw new Error(
            "The rotation operation does not contain a new commitment.",
          );
        }
  ...
  if (!network.proofsEnabled) {
    // Network.transaction captures true internally. Restore pending
    // authorizations after fromJSON reads the active network's actual mode.
    const original = transaction;
    transaction = Mina.Transaction.fromJSON(original.toJSON());
